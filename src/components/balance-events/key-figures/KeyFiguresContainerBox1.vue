<template>
  <v-hover v-slot="{ hover }">
    <fullscreen-viz
      class="viz-wrapper"
      viz-name="balance-events-key-figures-container-box-1"
      :drop-down-field="dropDownField"
      :is-hover="hover"
    >
      <qlik-viz
        name="balance-events-key-figures-container-box-1"
        :options="{ height: '100%' }"
  ...
  watch: {
    combined: {
      ...
      handler: async function() {
        if (this.combined) {
          await this.setFieldValList("kpi-dl");
        }
      }
    }
  }
